...
def plot_logs(target_date):
    ...
    try:
        ...
        # figure1 - cpu
        ax1.plot(df['timestamp'], df['cpu_percent'], label='CPU Usage (%)', color='#e74c3c', linewidth=1)
        ax1.set_ylabel('CPU %')
        ...
        # figure 2&3 - mem
        ax2.plot(df['timestamp'], df['memory_percent'], label='Memory Usage (%)', color='#3498db', linewidth=1)
        ax2.fill_between(df['timestamp'], df['memory_percent'], color='#3498db', alpha=0.2)
        ...
        ax3.plot(df['timestamp'], df['memory_used_gb'], label='Memory Used (GB)', color='#2ecc71', linewidth=1)
