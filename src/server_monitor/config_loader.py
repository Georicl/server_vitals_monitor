...
def load_config():
    ...
    if not os.path.exists(path_to_load):
        ...
        if os.path.exists(example_config_path):
            print(f"Example config file found at {example_config_path}. Please copy it to {path_to_load} and edit it."
                  f"Now using example config file.")
            
            path_to_load = example_config_path
        else:
            print(f"Can not find example config file at {example_config_path} or {path_to_load}. Please check your channel.")
            sys.exit(1)
